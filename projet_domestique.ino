...
void loop() {
  ...
  int lecturePotentio=analogRead(pinPotentio);
  float pourcentage=(lecturePotentio / 1023.0)*100.0;
  if(lecturePotentio > 511){
  ...
  if(temperature > consigne){
    ...
    if(lecturePotentio >511){
      Serial.println("Ventilateur tourne");
    }else{
      Serial.println("Veuillez allumer le ventillateur");
    }
  }else{
  ...
  if(digitalRead(btnB1) == LOW){
   ...
   if(digitalRead(sw1) == LOW){
    etatVentillateur =! etatVentillateur;
    digitalWrite(pinIN1, etatVentillateur);//les deux lignes c'est pour le sens direct
